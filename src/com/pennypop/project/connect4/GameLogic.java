...
public class GameLogic {
	 ...
	 * 
	 * @param x coordinate of selected player Cell
	 * @param y coordinate of selected player Cell
	 * @return true if there's a valid move based on player input, false otherwise
	 */
	public boolean placePiece(int x, int y){
		//Stop processing moves if game is over
		if(gameOver) return false;
		
		//We search the selected column for an available position to drop the piece starting from the bottom
		for (int i = 0; i < rows; i++) {
			if (board[x][i].isOccupied() == 0 ){
				//If a valid spot is found, set the Cell to the current player's color
	...
	public void reset(){
		...
		ProjectApplication.app.setScreen(newGame);
	}
	 ...
	 * 
	 * @param the victorious player. (-1 = Player 1, 1 = Player 2, 0 = y'all are losers)
	 */
	 ...
	 * 
	 * Win is met when either player accumulates more than 'n' consecutive pieces 
	 * on the board in vertical, horizontal, or diagonal orientation.
	 *  
	 * Is called after every player move
	 * 
	 * Simply takes the position of the most recently placed game piece and checks the 
	 * row, column, and diagonals branching from that piece for victory condition.
	 * 
	 * Note: This function doesn't really need to keep track of maxConsec
	 * But I was guessing max-consecutive pieces would be used as a heuristic for the AI to be implemented later
	 * 
	 * @param n how many consecutive pieces are needed for win. Usually set to 4
	 * @param x X coordinate of most recently placed piece
	 * @param y Y coordinate of most recently placed piece
